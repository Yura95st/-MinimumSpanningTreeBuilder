...
public class VoronoiDiagramGenerator
{
	...
	public VoronoiDiagramGenerator(List<Point> points, double width,
		double height)
	{
		this._points = points;
		this._width = width;
	...
	private void addParabola(Point point)
	{
		...
		Point start = new Point(point.getX(), this.getY(par.getPoint(), point.getX()));
	...
	private void checkCircleEvent(Parabola b)
	{
		...
		// add circle event
		ParabolaEvent e = new ParabolaEvent(new Point(start.getX(), start.getY() + d));
		e.arch = b;
		b.event = e;
		this._events.add(e);
	}
